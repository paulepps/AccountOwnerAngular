...
export class OwnerUpdateComponent implements OnInit {
  ...
  private getOwnerById = () => {
    ...
    this.repository.getOwner(ownerByIdUri).subscribe({
      next: (own: Owner) => {
        this.owner = {
          ...own,
          dateOfBirth: new Date(
            this.datePipe.transform(own.dateOfBirth, 'MM/dd/yyyy')
          ),
        };
  ...
  public updateOwner = (ownerFormValue) => {
    if (this.ownerForm.valid) this.executeOwnerUpdate(ownerFormValue);
  };
  ...
  private executeOwnerUpdate = (ownerFormValue) => {
    ...
    this.repository.updateOwner(apiUri, ownerForUpd).subscribe({
      next: (_) => {
